...
<html lang="en">
  ...
  <body class="bg-surface selection:bg-secondary-fixed selection:text-on-secondary-fixed">
    ...
    <main class="max-w-[1200px] mx-auto px-10 pt-16 pb-24">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        ...
        <div class="flex items-center gap-3">
          <span class="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Showing {{ $events->count() }} of {{ $events->total() }} Events (Page {{ $events->currentPage() }} of {{ $events->lastPage() }})</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @forelse($events as $event)
          <!-- Event Card -->
          <div class="event-card card-hover-effect bg-surface-container-lowest border border-surface-container rounded-2xl p-8 flex flex-col h-full group relative" data-status="{{ $event->status }}">
            <!-- Dropdown Menu (outside the link) -->
            <div class="absolute top-8 right-8">
              <div class="relative group">
                <button class="event-menu-btn p-2 hover:bg-surface-container rounded-lg transition-colors text-primary/50 hover:text-primary" onclick="event.stopPropagation(); toggleEventMenu(this)" data-event-id="{{ $event->e_id }}">
                ...
                <div class="event-menu hidden absolute right-0 mt-1 w-48 bg-surface-container-lowest border border-outline rounded-lg shadow-lg z-50">
                  <button class="w-full text-left px-4 py-3 text-sm font-semibold text-on-surface hover:bg-surface-container border-b border-outline flex items-center gap-2 transition-colors archive-btn" onclick="event.stopPropagation(); archiveEvent('{{ $event->e_id }}')"> 
                    <span class="material-symbols-outlined text-lg">archive</span>
                    Archive Event
                  </button>
                  <button class="w-full text-left px-4 py-3 text-sm font-semibold text-error hover:bg-error/10 flex items-center gap-2 transition-colors delete-btn" onclick="event.stopPropagation(); deleteEvent('{{ $event->e_id }}')"> 
            ...
            <!-- Content link -->
            <a href="{{ route('event-detail', ['eventId' => $event->e_id]) }}" class="flex-1 flex flex-col cursor-pointer">
            ...
            <h3 class="text-2xl font-bold font-headline text-primary mb-3 leading-tight">{{ $event->e_name }}</h3>
            <div class="flex items-center gap-2 text-on-surface-variant mb-10">
              <span class="material-symbols-outlined text-base">calendar_today</span>
              <span class="text-sm font-medium">{{ \Carbon\Carbon::parse($event->start_date)->format('M d, Y') }} • {{ \Carbon\Carbon::parse($event->start_time)->format('h:i A') }}</span>
            </div>
            <!-- Managed by section - shown when session is active -->
            <div class="managed-by-section mb-6 pb-6 border-b border-surface-container-high hidden" data-event-id="{{ $event->e_id }}">
            ...
            <div class="mt-auto pt-8 grid grid-cols-2 gap-4">
              <div class="flex flex-col">
                ...
                <p class="text-3xl font-black font-headline text-primary">{{ $event->attendancePercentage }}%</p>
              </div>
              <div class="flex flex-col">
                <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold mb-1">Students</p>
                <p class="text-xl font-bold font-headline text-on-surface">{{ $event->totalStudents }}</p>
              </div>
              <div class="col-span-2 mt-4">
                <div class="w-full h-2 bg-surface-container rounded-full overflow-hidden">
                  <div class="h-full bg-primary rounded-full progress-bar" data-percentage="{{ $event->attendancePercentage }}"></div>
                </div>
              </div>
            </div>
            </a>
        ...
        @empty
          ...
          <div class="col-span-full">
            <div class="text-center py-12 bg-surface-container-lowest rounded-2xl border border-surface-container">
              ...
              <a href="{{ route('schedule-event') }}" class="inline-flex items-center gap-2 bg-primary text-on-primary px-6 py-2.5 rounded-xl font-headline text-sm font-bold tracking-tight hover:opacity-80 transition-all">
        ...
        @if(Auth::user()->role === 'admin')
          <div class="border-2 border-dashed border-outline-variant/30 rounded-2xl flex flex-col items-center justify-center p-12 text-center group cursor-pointer hover:border-primary/40 hover:bg-primary/5 transition-all duration-300">
            <a href="{{ route('schedule-event') }}" class="w-full h-full flex flex-col items-center justify-center">
      ...
      @if ($events->hasPages())
        <div class="mt-12 mb-16">
          <nav class="flex flex-col md:flex-row items-center justify-center gap-2">
            ...
            @else
              <a href="{{ $events->previousPageUrl() }}" class="px-3 py-2 text-sm text-primary font-bold hover:bg-primary/10 rounded-lg transition-all">← Previous</a>
            @endif

            <div class="flex gap-1">
              @foreach ($events->getUrlRange(1, $events->lastPage()) as $page => $url)
                @if ($page == $events->currentPage())
                  <span class="px-3 py-2 text-sm font-bold text-on-primary bg-primary rounded-lg">{{ $page }}</span>
                @else
                  <a href="{{ $url }}" class="px-3 py-2 text-sm text-primary hover:bg-primary/10 rounded-lg transition-all">{{ $page }}</a>
                @endif
              @endforeach
            </div>

            @if ($events->hasMorePages())
              <a href="{{ $events->nextPageUrl() }}" class="px-3 py-2 text-sm text-primary font-bold hover:bg-primary/10 rounded-lg transition-all">Next →</a>
            @else
    ...
    <!-- Contextual FAB -->
    <a href="{{ route('schedule-event') }}" class="fixed bottom-10 right-10 w-16 h-16 bg-primary text-on-primary rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-300 z-50">
